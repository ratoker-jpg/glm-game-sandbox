...
if (window.FE_EXTERNAL_RENDER_DEBUG_ENABLED) {
  ...
  // FE_LT_04C3_ATTACK_MOVE_START
  function clearLightTankAttackMove(unit, keepMove=false) {
    if (!unit) return;
    unit.attackMoveTarget = null;
    unit.attackMoveAggroTargetId = null;
    unit.attackMoveMode = false;

    if (!keepMove && unit.state === 'attack_move') {
      unit.state = 'idle';
    }
  }
  ...
  function setGroupLightTankAttackMove(units, tx, ty) {
    ...
    for (let i = 0; i < ordered.length; i++) {
      ...
      if (setLightTankAttackMove(unit, cell.x, cell.y)) {
        accepted += 1;
      }
    }
  ...
  function resumeLightTankAttackMove(unit) {
    if (!isLightTank(unit) || !unit.attackMoveMode || !unit.attackMoveTarget) return false;

    const target = unit.attackMoveTarget;

    if (Math.round(unit.x) === Math.round(target.x) && Math.round(unit.y) === Math.round(target.y)) {
      clearLightTankAttackMove(unit);
      return false;
    }

    const path = findPath(unit, target, unit.id);
    if (path === null) {
      clearLightTankAttackMove(unit);
      return false;
    }
  ...
  function updateLightTankAttackMove(unit, dt) {
    if (!isLightTank(unit) || !unit.attackMoveMode) return false;
    ...
    const currentAggro = findUnitById(unit.attackMoveAggroTargetId);

    if (currentAggro && isEnemyUnit(currentAggro) && unitDistanceCells(unit, currentAggro) <= getLightTankCombatStats(unit).range) {
      unit.attackTargetId = currentAggro.id;
      unit.state = 'attacking';
      updateLightTankCombat(unit, dt);

      if (!findUnitById(currentAggro.id)) {
        resumeLightTankAttackMove(unit);
      }
    ...
    if (Math.round(unit.x) === Math.round(unit.attackMoveTarget.x) && Math.round(unit.y) === Math.round(unit.attackMoveTarget.y)) {
      clearLightTankAttackMove(unit);
      return true;
    }
  ...
  function onCanvasClick(e) {
    ...
    // Manual move / attack-move to free tile.
    if (selected?.kind==='unit') {
...
window.addEventListener('keydown', e=>{
  ...
  if ((e.key === 'a' || e.key === 'A' || e.code === 'KeyA') && game?.screen === 'game' && !game.paused) {
    ...
    if (units.length > 0) {
      ...
      hideMenus();
      showToast('Attack-move: выбери точку');
    } else {
      attackMoveArmed = false;
      showToast('Выбери light_tank для attack-move');
    }
    return;
  }
