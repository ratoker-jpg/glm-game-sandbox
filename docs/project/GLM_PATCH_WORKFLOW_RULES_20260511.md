...
5. Работай строго в двух фазах:
   ...
   - Фаза 2: реализация начинается только из отдельной явной задачи на реализацию после проверки аудита.
```
...
8. Manual test plan.
9. Ожидание проверки: «Аудит завершён. Готово для проверки GPT.»

> **Note:** Не требуй фиксированной фразы подтверждения. Не начинай реализацию автоматически после аудита. Для начала реализации нужна отдельная явная задача/промпт на реализацию.
```

После Фазы 1 пользователь отправляет анализ в GPT-аудит. Реализация начинается только из отдельной явной задачи на реализацию, после подтверждения GPT и решения Denis.
