(() => {
  ...
  // ============================================================
  let assets = {};
  let FE_LT_04C5FactionAssetsCache = Object.create(null);
  let game = null;
  let selected = null;
  let selectedUnits = [];
  ...
  function loadAssets(faction='cyan') {
    ...
    FE_LT_04C5FactionAssetsCache[faction] = assets;
  }
  ...
  // FE_LT_04C5_ENEMY_FACTION_VISUALS_START
  function FE_LT_04C5PickEnemyVisualFaction() {
    const playerFaction = game?.faction || 'cyan';
    const fallbackMap = {
      green: 'cyan',
      cyan: 'green',
      yellow: 'purple',
    ...
    return fallbackMap[playerFaction] ||
      Object.keys(FACTIONS).find(f => f !== playerFaction) ||
      'green';
  }

  function FE_LT_04C5GetFactionRenderAssets(faction) {
    if (!faction || faction === game?.faction) return assets;

    if (!FE_LT_04C5FactionAssetsCache[faction]) {
      FE_LT_04C5FactionAssetsCache[faction] = window.FE_ASSET_LOADER.loadAssets(faction);
    }

    return FE_LT_04C5FactionAssetsCache[faction];
  }

  function FE_LT_04C5UnitVisualFaction(unit) {
    if (unit?.visualFaction && FACTIONS[unit.visualFaction]) {
      return unit.visualFaction;
    }

    if (isLightTank(unit) && isEnemyUnit(unit)) {
      return FE_LT_04C5PickEnemyVisualFaction();
    }

    return game?.faction || 'cyan';
  }
  // FE_LT_04C5_ENEMY_FACTION_VISUALS_END
  ...
  function setLightTankAttack(attacker, target) {
    ...
    const distance = unitDistanceCells(attacker, target);

    if (distance > stats.range) {
      addUnitClickMarker(attacker, Math.round(target.x), Math.round(target.y), 'bad');
    ...
    addUnitClickMarker(attacker, Math.round(target.x), Math.round(target.y), 'ok');
  ...
  function updateLightTankCombat(unit, dt) {
    ...
    const target = findUnitById(unit.attackTargetId);
    if (!target || !isLightTank(target) || unitOwner(target) === unitOwner(unit)) {
    ...
    const distance = unitDistanceCells(unit, target);
    if (distance > stats.range) {
      unit.attackTargetId = null;
      unit.attackCooldown = 0;
      unit._attackCommanded = false;
    ...
    if (unit.attackCooldown <= 0) {
      const killed = damageUnit(target, stats.damage);
  ...
  function findLightTankSpawnCell(owner='enemy', index=0) {
    const hq = game?.buildings?.find(b => b.type === 'hq_base') || null;
    const start = hq
  ...
  function spawnLightTankForOwner(owner='enemy', count=1) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      tank.owner = owner;
      if (owner === 'enemy') {
        tank.visualFaction = FE_LT_04C5PickEnemyVisualFaction();
      }
...
const renderAssets = FE_LT_04C5GetFactionRenderAssets(FE_LT_04C5UnitVisualFaction(u));
...
drawSprite(unitImage, rx, ry, profile.size, {
  ...
  anchorY: profile.anchorY ?? 1.0
    ...
    ctx.fillStyle = isLightTank(u) && isEnemyUnit(u)
      ? '#ff5b5b'
      : unitSelected
        ? '#fff08a'
        : '#60e870';
...
if (window.FE_EXTERNAL_RENDER_DEBUG_ENABLED) {
  ...
  function drawUnitSelectionRingAt(x, y, active=false) {
    // FE_LT_04C2G_SELECTION_VISUAL_FIX
    // FE_LT_04C2G2_DRAW_SELECTION_GLOW_UNDER_UNIT
    // FE_LT_04C2G3_SELECTION_GLOW_REAL_ANCHOR
    const z = game?.camera?.zoom || 1;
    const rX = Math.max(30, TILE_W * z * 0.54);
    const rY = Math.max(11, TILE_H * z * 0.28);

    ctx.save();

    const outer = ctx.createRadialGradient(x, y, 1, x, y, rX);
    outer.addColorStop(0.00, active ? 'rgba(255, 68, 196, 0.58)' : 'rgba(255, 88, 180, 0.44)');
    outer.addColorStop(0.50, active ? 'rgba(255, 68, 196, 0.32)' : 'rgba(255, 88, 180, 0.22)');
    outer.addColorStop(1.00, 'rgba(255, 88, 180, 0.00)');
    ...
    const inner = ctx.createRadialGradient(x, y, 1, x, y, rX * 0.60);
    inner.addColorStop(0.00, active ? 'rgba(255, 236, 126, 0.38)' : 'rgba(255, 182, 214, 0.28)');
    inner.addColorStop(1.00, 'rgba(255, 182, 214, 0.00)');

    ctx.fillStyle = inner;
    ctx.beginPath();
    ctx.ellipse(x, y, rX * 0.58, rY * 0.56, 0, 0, Math.PI * 2);
    ctx.fill();
  ...
  function findNearestLightTankDestinationCell(tx, ty, unitId=null, reservedKeys=null) {
    for (const [dx, dy] of lightTankDestinationOffsets()) {
      ...
      if (!lightTankDestinationCellFree(x, y, unitId, reservedKeys)) continue;
  ...
  function groupMoveDestinationCells(tx, ty, count) {
    const result = [];
  ...
  function setGroupManualMove(units, tx, ty) {
    ...
    const cells = groupMoveDestinationCells(tx, ty, group.length);
  ...
  function drawSelectedUnitRingFor(unit, active=false, anchorX=null, anchorY=null) {
    ...
    if (Number.isFinite(anchorX) && Number.isFinite(anchorY)) {
      drawUnitSelectionRingAt(anchorX, anchorY - 6 * z, active);
      return;
    }

    const p = tileToScreen(unit.x, unit.y);
    drawUnitSelectionRingAt(p.x, p.y + TILE_H * z * 0.58, active);
  }
  ...
  function setLightTankAttackApproach(attacker, target) {
    ...
    if (unitDistanceCells(attacker, target) <= getLightTankCombatStats(attacker).range) {
    ...
    for (const cell of cells) {
      ...
      attacker._attackCommanded = false;
      attacker.path = path;
  ...
  function updateLightTankAttackApproach(unit, dt) {
    ...
    const target = findUnitById(unit.attackApproachTargetId);

    if (!target || !isLightTank(target) || !isEnemyUnit(target)) {
      clearLightTankAttackApproach(unit);
      if (unit.state === 'attack_approach') unit.state = 'idle';
      return true;
    }

    if (unitDistanceCells(unit, target) <= getLightTankCombatStats(unit).range) {
      clearLightTankAttackApproach(unit);
      setLightTankAttack(unit, target);
      return true;
    }
  ...
  function setLightTankAttackMove(unit, tx, ty) {
    ...
    const path = findPath(unit, cell, unit.id);
    if (path === null) {
      addUnitClickMarker(unit, cell.x, cell.y, 'bad');
      return false;
    }
    ...
    addUnitClickMarker(unit, cell.x, cell.y, 'ok');
    return true;
  }
  ...
  function setGroupLightTankAttackMove(units, tx, ty) {
    ...
    const cells = groupMoveDestinationCells(tx, ty, group.length);
    if (!cells.length) {
      showToast('Р“СЂСѓРїРїР° РЅРµ РјРѕР¶РµС‚ РІС‹РїРѕР»РЅРёС‚СЊ attack-move');
      return true;
    }

    const ordered = group.slice().sort((a, b) => dist(a, {x:tx,y:ty}) - dist(b, {x:tx,y:ty}));
    let accepted = 0;
    ...
    for (let i = 0; i < ordered.length; i++) {
      ...
      if (setLightTankAttackMove(unit, cell.x, cell.y)) accepted += 1;
    }
    ...
    if (selected?.kind==='unit') {
      ...
      if (attackMoveArmed && attackMoveUnits.length > 0) {
        if (attackMoveUnits.length > 1) {
          ...
          showToast(ok ? 'Attack-move' : 'РўР°РЅРє РЅРµ РјРѕР¶РµС‚ РІС‹РїРѕР»РЅРёС‚СЊ attack-move');
        }
